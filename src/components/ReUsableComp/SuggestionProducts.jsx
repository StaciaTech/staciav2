...
const SuggestionProduct = ({ currentProductId }) => {
  const scrollRef = useRef(null);
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(false);
  ...
  const scrollCard = (direction) => {
    if (scrollRef.current) {
      const cardWidth =
        scrollRef.current.querySelector(".suggestion-casestudy-card-1")
          ?.offsetWidth || 397;
      const gap = 24;
      const scrollAmount = cardWidth + gap;
      scrollRef.current.scrollBy({
  ...
  return (
    <div className="suggestion-casestudys-container-1">
      <div className="header-with-arrows-1">
        <h2 className="suggestion-casestudys-title-1">Suggestion Products</h2>
        {allProducts.length > 1 && (
          <div className="carousel-controls">
            <button
              className="carousel-arrow carousel-arrow-left-"
