...
function Popup() {
  ...
  const [show, setShow] = useState(() => {
    try {
      ...
      // if never shown (last===0) or more than an hour since last, show
      return Date.now() - last >= ONE_DAY_MS;
    } catch (e) {
      // in case localStorage is not available (SSR or privacy mode), default to showing
      return true;
    }
  });
  ...
  // Carousel / timers (kept same timing as original)
  useEffect(() => {
    let timeout;
    if (showContent2) {
      timeout = setTimeout(() => {
  ...
  useEffect(() => {
    const id = setInterval(() => setProjectIndex((p) => p + 1), 4000);
    return () => clearInterval(id);
  }, []);

  useEffect(() => {
    const id = setInterval(() => setServiceIndex((p) => p + 1), 7000);
    return () => clearInterval(id);
  }, []);

  useEffect(() => {
    const id = setInterval(() => setAchievementIndex((p) => p + 1), 5000);
    return () => clearInterval(id);
  }, []);

  useEffect(() => {
    const id = setInterval(
      () => setProductIndex((p) => (p + 1) % productSlides.length),
      4000
    );
    return () => clearInterval(id);
  }, [productSlides.length]);

  // reset the content2 box when index changes
  useEffect(() => {
    setShowContent2Box5(false);
  }, [currentIndex]);
  ...
  // unified close function that also stores last-shown timestamp
  const closePopup = () => {
    try {
  ...
  // attach outside click only when popup is shown
  useEffect(() => {
    if (!show) return;
  ...
  // If mobile, render PopupMobile (you can handle mobile show frequency inside PopupMobile if desired)
  if (isMobile) {
    return <PopupMobile />;
  }
