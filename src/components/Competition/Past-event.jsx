...
const PastEventCard = ({
  ...
  return (
    <div className="past-event-card">
      <div>
        <img src={imageUrl} className="event-img" alt="Event" loading="lazy" />
      </div>
      <div className="event-content">
        <span className="event-tag">{eventTags || "No Tags"}</span>
        <h3 className="event-title-past">{title || "Untitled Event"}</h3>
        ...
        <p className="event-date">
          <img src={dateIcon} alt="Date" className="date-icon" loading="lazy" />
          <h4>{date ? formatDate(date) : "No date available"}</h4>
        </p>
        <button
          onClick={() => {
        ...
        >
          Read More
          <img src={vector} alt="Arrow" className="arrow-icon" loading="lazy" />
        </button>
      </div>
    </div>
  );
...
export default function LazyPastEventsList() {
  return (
